...
WWW-Authenticate: Basic realm="ngrok-thur"
Content-Length: 23
...
func httpHandler(c conn.Conn, proto string) {
	...
	//验证代理权限
	fmt.Println("处理proto:",proto)
	fmt.Println("处理,Request:",vhostConn.Request)
	fmt.Println("处理,host:",vhostConn.Host())
	fmt.Println("处理,LocalAddr:",vhostConn.LocalAddr())
