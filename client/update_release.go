...
func applyUpdate(s mvc.State, result *check.Result) {
	...
	log.Error("Error while updating ngrok-thur: %v", err)
	if errRecover != nil {
		log.Error("Error while recovering from failed ngrok-thur update, your binary may be missing: %v", errRecover.Error())
	}
